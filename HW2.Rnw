...
point.for.trap = a + 1:99*(width.of.trap)

#Formula for the trapezoid

0.5*width.of.trap*sum(integrand(a) + integrand(b) + sum(2*integrand(point.for.trap)))
...
riemann.sums <- function(fnct,                        # function to integrate
  ...
  if(method == "Left"){
    ...
    area = 0.5*delta.x*sum(fnct(a) + fnct(b) + sum(2*fnct(point.for.trap)))
  }else{
