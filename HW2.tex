...
\hldef{point.for.trap} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{100}\hlopt{*}\hldef{(width.of.trap)}

\hlcom{#Formula for the trapezoid}

\hlnum{0.5}\hlopt{*}\hldef{width.of.trap}\hlopt{*}\hlkwd{sum}\hldef{(}\hlkwd{integrand}\hldef{(a)} \hlopt{+} \hlkwd{integrand}\hldef{(b)} \hlopt{+} \hlkwd{sum}\hldef{(}\hlnum{2}\hlopt{*}\hlkwd{integrand}\hldef{(a}\hlopt{+}\hldef{point.for.trap)))}
\end{alltt}
\begin{verbatim}
## [1] 8.6464
\end{verbatim}
