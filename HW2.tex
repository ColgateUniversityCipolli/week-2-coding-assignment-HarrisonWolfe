...
\hlnum{0.5}\hlopt{*}\hldef{width.of.trap}\hlopt{*}\hlkwd{sum}\hldef{(}\hlkwd{integrand}\hldef{(a)} \hlopt{+} \hlkwd{integrand}\hldef{(b)} \hlopt{+} \hlkwd{sum}\hldef{(}\hlnum{2}\hlopt{*}\hlkwd{integrand}\hldef{(point.for.trap)))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    ...
    \hldef{area} \hlkwb{=} \hlnum{0.5}\hlopt{*}\hldef{delta.x}\hlopt{*}\hlkwd{sum}\hldef{(}\hlkwd{fnct}\hldef{(a)} \hlopt{+} \hlkwd{fnct}\hldef{(b)} \hlopt{+} \hlkwd{sum}\hldef{(}\hlnum{2}\hlopt{*}\hlkwd{fnct}\hldef{(point.for.trap)))}
